...
function App() {
    ...
    try {
      ...
      if (adData) {
        setAdvertisements(adData.map(ad => ({
          ...
          size: ad.size, customWidth: ad.customWidth, customHeight: ad.customHeight, placement: ad.placement, targetCategory: ad.targetCategory || ad.target_category,
  ...
  // Get all pending devices for the current user
  const pendingMyDevices = devices.filter(d => d.status === 'pending' && d.userId === userId);
  // For the popup, we still just show the first one
  const pendingDevice = pendingMyDevices[0];
  ...
  return (
    ...
    <Layout 
      ...
      articles={articles} 
      categories={categories}
      // Only pass pending devices if current device is Primary
      pendingDevices={isPrimary ? pendingMyDevices : []}
