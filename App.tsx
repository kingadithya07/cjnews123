...
// Fixed UUID for global settings to ensure compatibility with Postgres UUID types
...
function App() {
  ...
  // Track userId in a ref for safe use in realtime subscription callbacks
  ...
  const handleLogActivity = async (action: ActivityLog['action'], details?: string) => {
      ...
      const newLog: ActivityLog = { id: generateId(), userId: userIdRef.current, deviceName: meta.name, action, details: details || '', ip, location: 'Local Network', timestamp: new Date().toISOString() };
      ...
      } catch (e) { console.warn("Activity log failed to sync."); }
  };
  ...
  const fetchLogs = async () => {
      ...
      try {
          ...
          if (data) setActivityLogs(data.map((l: any) => ({ id: l.id, userId: l.user_id, deviceName: l.device_name, action: l.action, details: l.details, ip: l.ip, location: l.location, timestamp: l.timestamp })));
      } catch (e) { console.warn("Logs unavailable."); }
  };
  ...
  const fetchData = async (force: boolean = false) => {
    try {
      ...
      if (adData) setAdvertisements(adData.map(ad => ({ id: ad.id, imageUrl: ad.image_url || ad.imageUrl, linkUrl: ad.link_url || ad.linkUrl, title: ad.title, size: ad.size, placement: ad.placement, targetCategory: ad.target_category || ad.targetCategory, isActive: ad.is_active !== undefined ? ad.is_active : (ad.isActive !== undefined ? ad.isActive : true) })));
      setLastSync(new Date());
    } catch (err) { console.error("Global data sync failed."); }
  };
  ...
  useEffect(() => {
    fetchData();
    const sub = supabase.channel('global_db_sync').on('postgres_changes', { event: '*', schema: 'public' }, () => fetchData(true)).subscribe();
    supabase.auth.getSession().then(({ data: { session } }) => {
        if (session) { setUserId(session.user.id); setUserName(session.user.user_metadata.full_name); setUserRole(session.user.user_metadata.role || UserRole.READER); fetchDevices(session.user.id); fetchLogs(); }
        setLoading(false);
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
        if (session) { setUserId(session.user.id); setUserName(session.user.user_metadata.full_name); setUserRole(session.user.user_metadata.role || UserRole.READER); }
        else { setUserId(null); setUserName(null); setUserRole(UserRole.READER); }
    });
  ...
  const handleSaveArticle = async (article: Article) => {
    ...
    await supabase.from('articles').upsert(payload);
    handleLogActivity('EDIT', `Dispatch: ${article.title.substring(0, 20)}...`);
    fetchData(true);
  };
  ...
  />;
    ...
    content = <ArticleView articles={articles} articleId={found?.id} onNavigate={navigate} advertisements={advertisements} globalAdsEnabled={globalAdsEnabled} />;
  ...
  return (
    <>
      <Layout currentRole={userRole} onRoleChange={setUserRole} currentPath={currentPath} onNavigate={navigate} userName={userName} userAvatar={userAvatar} onForceSync={() => fetchData(true)} lastSync={lastSync} articles={articles} categories={categories} pendingDevices={devices.filter(d => d.status === 'pending' && d.userId === userId)} onApproveDevice={id => handleUpdateDeviceStatus(id, 'approved')} onRejectDevice={handleRevokeDevice}>
        {content}
      </Layout>
             ...
             <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-red-100">
                <div className="bg-red-600 px-6 py-4 flex items-center gap-3">
                   ...
                   <div>
                      ...
                      <p className="text-red-100 text-xs">Awaiting Authorization</p>
                   </div>
                </div>
                <div className="p-6">
                   <div className="space-y-6">
                        <div className="text-center"><p className="text-gray-600 text-sm">A new device is requesting access to your publishing station.</p></div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <button onClick={() => handleUpdateDeviceStatus(pendingDevice.id, 'approved')} className="py-3 rounded-lg bg-green-600 text-white font-bold uppercase text-xs hover:bg-green-700 shadow-lg transition-colors flex items-center justify-center gap-2"><Check size={16}/> Approve Station</button>
                        </div>
                   </div>
                </div>
             </div>
