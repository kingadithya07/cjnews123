...
const WriterDashboard: React.FC<WriterDashboardProps> = ({ 
  ...
  return (
    <>
    <ImageGalleryModal isOpen={showImageGallery} onClose={() => setShowImageGallery(false)} onSelectImage={setImageUrl} uploadFolder="articles" userId={userId} />
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#1a1a1a] text-white flex flex-col transition-transform duration-300 shadow-2xl ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          ...
          <div className="flex-1 overflow-y-auto py-4">
              <SidebarItem id="articles" label="My Articles" icon={FileText} />
              <SidebarItem id="analytics" label="Performance" icon={BarChart3} />
              <SidebarItem id="settings" label="Staff Settings" icon={Settings} />
          </div>
              ...
              {activeTab === 'articles' && (
                  <div className="max-w-6xl mx-auto space-y-6">
                      <div className="flex justify-between items-center mb-6">
                           ...
                           <button onClick={openNewArticle} className="bg-news-black text-white text-xs font-bold uppercase px-4 py-2 rounded flex items-center gap-2"><Plus size={16} /> New Draft</button>
                      </div>
                      <div className="bg-white rounded border overflow-x-auto shadow-sm">
                          <table className="w-full text-left min-w-[700px]">
                                <thead className="bg-gray-50 text-gray-500 text-xs font-bold uppercase">
                                    <tr><th className="px-6 py-4">Title</th><th className="px-6 py-4">Sections</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Actions</th></tr>
                                </thead>
                                <tbody className="divide-y">
                                    {myArticles.map((article) => (
                                        <tr key={article.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4"><span className="font-medium text-gray-900 text-sm truncate max-w-sm block">{article.title}</span></td>
                                            ...
                                            <td className="px-6 py-4 text-right flex justify-end gap-3">
                                                <button onClick={() => openEditArticle(article)} className="text-blue-600 font-bold text-xs uppercase hover:underline">Edit</button>
                                                {onDelete && <button onClick={() => onDelete(article.id)} className="text-red-500 font-bold text-xs uppercase hover:underline">Delete</button>}
                                            </td>
                                        </tr>
                                    ))}
                                    {myArticles.length === 0 && <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-400">No active drafts found.</td></tr>}
                                </tbody>
                          </table>
                      </div>
                  </div>
              ...
              {activeTab === 'settings' && (
                   <div className="max-w-4xl mx-auto space-y-12">
                       <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm">
                           <h3 className="font-bold text-lg flex items-center gap-2 mb-6"><ShieldCheck size={20} className="text-news-gold"/> Trusted Terminal Authorization</h3>
                               ...
                               {devices.map(device => (
                                   <div key={device.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-100">
                                       <div className="flex items-center gap-4">
                                            <div className="p-2 bg-white rounded border text-gray-500">{device.deviceType === 'mobile' ? <Smartphone size={18}/> : <Monitor size={18}/>}</div>
                                            <div>
                                                <span className="font-bold text-sm block">{device.deviceName} {device.isCurrent && <span className="text-[9px] text-blue-500">(Active Now)</span>}</span>
                                                <span className="text-[10px] text-gray-400 uppercase font-black tracking-widest">{device.browser} • {device.location}</span>
                                            </div>
                                       </div>
                                       {!device.isPrimary && onRevokeDevice && <button onClick={() => onRevokeDevice(device.id)} className="text-red-500 text-[10px] font-black uppercase tracking-widest hover:bg-red-50 p-2 rounded">De-authorize</button>}
                                   </div>
                               ))}
                           </div>
                       </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center p-4 animate-in zoom-in-95">
          <div className="bg-white rounded-lg w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
                <h3 className="font-bold text-gray-900">{activeArticleId ? 'Edit dispatch' : 'New dispatch'}</h3>
            ...
            <div className="p-4 md:p-6 overflow-y-auto space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 space-y-4">
                        ...
                        <textarea value={subline} onChange={e => setSubline(e.target.value)} className="w-full p-2 border rounded text-sm italic min-h-[80px]" placeholder="Summary..."></textarea>
                        <button onClick={() => setShowCategorySelector(true)} className="w-full p-2 border rounded text-sm bg-white text-left flex justify-between items-center">
                            <span className={selectedCategories.length === 0 ? 'text-gray-400' : ''}>{selectedCategories.length === 0 ? 'Select Categories' : `${selectedCategories.length} Selected`}</span>
                            <ChevronDown size={14} />
                        </button>
                    </div>
                    <div className="md:col-span-1">
                        <div className="border-2 border-dashed p-4 rounded bg-gray-50 text-center h-[200px] flex flex-col justify-between overflow-hidden">
                            {imageUrl ? <img src={imageUrl} className="max-h-32 mx-auto rounded shadow-sm" /> : <div className="text-gray-400 py-6"><ImageIcon size={32} className="mx-auto opacity-20" /><p className="text-[10px] font-black uppercase mt-2">No Image Picked</p></div>}
                            <button type="button" onClick={() => setShowImageGallery(true)} className="w-full bg-white border border-gray-300 hover:bg-gray-100 text-gray-700 text-xs font-bold px-2 py-2 rounded flex items-center justify-center gap-2 cursor-pointer transition-colors shadow-sm">
                                <Library size={14} /> <span>Pick from Gallery</span>
                            </button>
                        </div>
                    </div>
                </div>
                <RichTextEditor content={content} onChange={setContent} onImageUpload={async () => ''} className="min-h-[400px]" />
            </div>
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3 shrink-0">
              <button onClick={() => setShowEditorModal(false)} className="px-5 py-2 text-sm font-bold text-gray-600">Discard</button>
