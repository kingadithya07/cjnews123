...
const EPaperReader: React.FC<EPaperReaderProps> = ({ pages, onNavigate, watermarkSettings }) => {
  ...
  // Maps mouse position (0-100%) to image offset when zoomed
  const handleReaderMouseMove = (e: React.MouseEvent) => {
    if (scale > 1 && viewerRef.current && !isCropping) {
        ...
        // Total scrollable area is (container_size * scale) - container_size
        // We move from -(range/2) to +(range/2)
  ...
  const handleWorkshopMouseMove = (e: React.MouseEvent) => {
      if (isCropping && workshopScale > 1 && cropperRef.current && !cropPreview) {
          ...
          // Use cropper move function to simulate hover pan
          // This is a simplified calculation for the cropper internal canvas
          const rangeX = (imageData.width * workshopScale - containerData.width);
          const rangeY = (imageData.height * workshopScale - containerData.height);

          // We don't want to actually "move" the image data but scroll the view
          // Since cropper moves the image, we adjust based on percentage
          ...
          <div className="p-6 md:p-12 animate-in fade-in duration-500">
              <div className="max-w-7xl mx-auto space-y-12">
                  <div className="border-b border-gray-200 pb-8">
                      <h1 className="text-3xl md:text-5xl font-serif font-black text-gray-900 mb-2">Paper Archive</h1>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 md:gap-10">
                      {currentEditionPages.map((page, idx) => (
                          ...
                          >
                              <div className="relative aspect-[3/4] overflow-hidden rounded-md border border-gray-200 shadow-sm transition-all duration-500 group-hover:scale-[1.03] group-hover:shadow-xl">
                                  ...
                                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                                      <div className="bg-news-black text-white p-3 rounded-full opacity-0 group-hover:opacity-100 transition-opacity transform scale-75 group-hover:scale-100">
                                          <Maximize size={20} />
                                      </div>
                                  </div>
                              </div>
                              <p className="text-[10px] font-black tracking-[0.3em] text-gray-500 group-hover:text-news-accent transition-colors text-center">PAGE {page.pageNumber}</p>
                          </div>
                      ))}
                  </div>
              </div>
      ...
      {viewMode === 'reader' && (
          <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between px-4 md:px-8 py-3 md:py-4 bg-news-black border-b border-white/5 z-50 shrink-0 shadow-2xl">
                  <div className="flex items-center gap-3 md:gap-8">
                      <button 
                          ...
                          className="flex items-center gap-2 text-gray-400 hover:text-white transition-all bg-white/5 px-3 py-1.5 rounded-full"
                      >
                          <X size={18} />
                          <span className="text-[10px] font-black uppercase tracking-[0.1em]">CLOSE</span>
                      </button>
                      <div className="hidden md:flex items-center gap-2 border-l border-white/10 pl-6">
                           ...
                           <span className="text-gray-600 text-[10px] font-bold">PAGE {activePage?.pageNumber} OF {currentEditionPages.length}</span>
                      </div>
                  </div>

                  <div className="flex items-center gap-2 md:gap-4">
                      {scale > 1 && (
                          <div className="hidden md:flex items-center gap-2 text-news-gold px-3 py-1 bg-white/5 rounded-full border border-news-gold/20 animate-pulse">
                              <MousePointer2 size={12} />
                              <span className="text-[8px] font-black uppercase tracking-widest">Hover Mode Active</span>
                          </div>
                      )}
                      <button 
                          onClick={() => setIsCropping(true)}
                          className="flex items-center gap-2 px-5 md:px-8 py-2 md:py-2.5 bg-news-gold text-black rounded-full transition-all hover:bg-white shadow-lg active:scale-95"
                      >
                          <Scissors size={14} />
                          <span className="text-[10px] font-black uppercase tracking-[0.2em]">CROP</span>
                      </button>
                      
                      <div className="w-px h-6 bg-white/10 hidden md:block"></div>
                      
                      <div className="flex items-center gap-1 bg-white/5 rounded-full px-1">
                          <button onClick={() => setScale(s => Math.max(1, s - 0.5))} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ZoomOut size={16} /></button>
                          <span className="text-[9px] font-mono font-black text-news-gold w-10 text-center">{Math.round(scale * 100)}%</span>
                          <button onClick={() => setScale(s => Math.min(4, s + 0.5))} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ZoomIn size={16} /></button>
                      </div>
                  </div>
              </div>
              ...
              <div 
                  ...
                  className={`flex-1 relative flex items-center justify-center overflow-hidden bg-[#080808] ${scale > 1 ? 'cursor-none' : 'cursor-default'}`}
              >
                  {activePage ? (
                      <div 
                        ref={contentRef}
                      ...
                      >
                          <EPaperViewer 
                            page={activePage} 
                            imageClassName="max-h-[85vh] h-auto w-auto max-w-full block shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/5"
                      ...
                      <div className="flex flex-col items-center gap-6 opacity-30">
                          ...
                          <p className="font-black uppercase tracking-[0.5em] text-[10px]">Syncing Edition...</p>
                      </div>
                  )}

                  {/* Navigation Handles - Hidden when zoomed to prevent mistakes (Freeze state) */}
                  {scale === 1 && (
                    <>
                        <div className="absolute inset-y-0 left-0 flex items-center p-2">
                            <button 
                                ...
                                className="p-4 text-white/20 hover:text-white transition-colors disabled:opacity-0 bg-white/5 hover:bg-white/10 rounded-full"
                            >
                                <ChevronLeft size={32} />
                            </button>
                        </div>
                  ...
                  <button 
                    ...
                    className={`absolute bottom-6 right-6 p-4 bg-white/10 hover:bg-news-gold hover:text-black rounded-full transition-all shadow-2xl ${scale > 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-50 pointer-events-none'}`}
                    title="Zoom Out to Unlock"
                  >
                    <RotateCcw size={20} />
                  </button>

                  {/* Visual Indicator of Hover Navigation */}
                  {scale > 1 && (
                      <div className="absolute inset-0 border-4 border-news-gold/10 pointer-events-none z-10"></div>
                  )}
              </div>

              {/* Bottom Mobile Status */}
              <div className="md:hidden px-4 py-2 bg-black border-t border-white/5 text-center">
                  <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">Page {activePage?.pageNumber} of {currentEditionPages.length}</span>
              </div>
          </div>
      )}

      {/* CLIPPING WORKSHOP (FULL SCREEN OVERLAY) */}
      {isCropping && (
        <div className="fixed inset-0 z-[110] bg-[#050505] flex flex-col animate-in fade-in zoom-in-95 duration-300">
            {/* Workshop Header */}
            <div className="px-4 md:px-6 py-3 md:py-5 bg-news-black border-b border-white/5 flex items-center justify-between shrink-0 shadow-2xl">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[8px] text-gray-600 uppercase tracking-widest font-bold hidden md:block mt-1">Refining Borders</p>
                    </div>
                </div>
                ...
                <div className="flex gap-2 md:gap-3">
                    {cropPreview ? (
                        <div className="flex gap-2">
                            <button onClick={handleShare} className="bg-white/5 text-white border border-white/10 px-4 py-2 rounded-full hover:bg-white/10 transition-all flex items-center gap-2">
                                <Share2 size={16} /> <span className="text-[9px] font-black uppercase tracking-[0.2em]">Share</span>
                            </button>
                            <button onClick={handleDownload} className="bg-news-gold text-black px-6 py-2 rounded-full hover:bg-yellow-500 transition-all flex items-center gap-2 shadow-xl">
                                <Download size={16} /> <span className="text-[9px] font-black uppercase tracking-[0.2em]">Save</span>
                            </button>
                        ...
                        <div className="flex gap-3">
                            <div className="hidden md:flex items-center gap-2 bg-white/5 rounded-full px-2 border border-white/10 mr-4">
                                <button onClick={() => cropperRef.current?.zoom(-0.2)} className="p-2 hover:bg-white/10 text-white rounded-full"><ZoomOut size={16} /></button>
                                <span className="text-[9px] font-mono font-black text-news-gold w-10 text-center">{Math.round(workshopScale * 100)}%</span>
                                <button onClick={() => cropperRef.current?.zoom(0.2)} className="p-2 hover:bg-white/10 text-white rounded-full"><ZoomIn size={16} /></button>
                            </div>
                            <button 
                                onClick={generateBrandedClip}
                                disabled={isProcessing}
                                className="bg-news-gold text-black px-6 md:px-10 py-2.5 md:py-3 rounded-full flex items-center gap-2 hover:bg-yellow-500 transition-all shadow-xl disabled:opacity-50"
                            >
                                {isProcessing ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} 
                                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em]">{isProcessing ? 'Processing...' : 'Apply Crop'}</span>
                            </button>
                        </div>
                    )}
                </div>
            ...
            <div 
                ...
                className={`flex-1 overflow-hidden relative flex flex-col ${cropPreview ? 'md:flex-row' : 'flex-col'} bg-[#080808]`}
            >
                <div className={`relative flex flex-col items-center justify-center transition-all duration-500 ${cropPreview ? 'w-full md:w-1/2 border-r border-white/5 h-1/2 md:h-full p-4' : 'w-full h-full p-2 md:p-8'}`}>
                    {workshopScale > 1 && !cropPreview && (
                        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-news-gold text-black px-4 py-1.5 rounded-full font-black uppercase text-[8px] tracking-[0.25em] shadow-2xl animate-bounce">
                           <MousePointer2 size={12} /> Panning Enabled • Zoom Out to Lock
                        </div>
                    )}
                ...
                {cropPreview && (
                    <div className="w-full md:w-1/2 h-1/2 md:h-full flex flex-col items-center justify-center p-6 md:p-16 bg-[#0c0c0c] animate-in slide-in-from-bottom md:slide-in-from-right duration-700 relative">
                        <div className="absolute top-4 right-6 text-[8px] font-black uppercase tracking-[0.4em] text-news-gold">PREVIEW</div>
