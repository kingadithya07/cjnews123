...
const ArticleView: React.FC<ArticleViewProps> = ({ articles = [], articleId, onNavigate, advertisements = [], globalAdsEnabled }) => {
  ...
  const handleShare = async () => {
      ...
      // Construct a detailed text caption including Title, Subline, and URL.
      // We add explicit newlines to separate the sections visually in apps like WhatsApp.
      const captionText = `${article.title}\n\n${article.subline ? article.subline + '\n\n' : ''}🔗 Read more: ${permalink}`;
      ...
      if (navigator.share) {
          try {
              ...
              if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
                  ...
                  // IMPORTANT: We do NOT pass the 'url' property here. 
                  // If 'url' is passed with 'files', Android/WhatsApp often ignores 'text' and just sends the file + url.
                  // By putting the URL inside 'text' and omitting 'url', we force the app to use 'text' as the image caption.
